...
function sparql_construct( $sparql, $headerAccept, $db = null ) { return _sparql_a_connection( $db )->construct( $sparql, $headerAccept ); } //RATHACHAI
...
class sparql_connection
{
	...
	function construct( $query, $headerAccept, $timeout=null) // RATHACHAI
	{	
		$prefixes = "";
		foreach( $this->ns as $k=>$v )
		{
		...
		$output = $this->dispatchConstruct( $prefixes.$query , $headerAccept, $timeout);
		if( $this->errno ) { return; }
		return $output;
	}
	...
	function dispatchConstruct( $sparql, $headerAccept, $timeout=null) //RATHACHAI
	{
		$url = $this->endpoint."?query=".urlencode( $sparql );
		if( $this->params !== null )
		{
